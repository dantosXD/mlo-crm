...
try {
  ...
  // Pre-hashed password for "Test1234!" using bcrypt
  // You can verify: bcrypt.hashSync("Test1234!", 12)
  const passwordHash = '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5NU9bK8fk.5lq';

  // Check if user already exists
  const existing = db.prepare('SELECT id FROM users WHERE email = ?').get('feature100-test@example.com');
