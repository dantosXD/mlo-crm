...
/**
 * Flow control action configuration
 */
interface FlowControlActionConfig {
  ...
  delayHours?: number;
  delayDays?: number;
  delayUntil?: Date; // Specific datetime to wait until

  // BRANCH action config
  condition?: string; // Condition expression to evaluate
  trueActions?: any[]; // Actions to execute if condition is true
  falseActions?: any[]; // Actions to execute if condition is false
  variable?: string; // Variable to check (e.g., client.status, document.status)
  operator?: string; // Comparison operator: equals, not_equals, contains, greater_than, less_than
  value?: any; // Value to compare against

  // PARALLEL action config
  actions?: any[]; // Array of actions to execute in parallel
  continueOnError?: boolean; // Whether to continue if one action fails
}
...
export async function executeWait(
  ...
  try {
    ...
    if (config.delayMinutes) {
      delayMs = config.delayMinutes * 60 * 1000;
    } else if (config.delayHours) {
      delayMs = config.delayHours * 60 * 60 * 1000;
    } else if (config.delayDays) {
      delayMs = config.delayDays * 24 * 60 * 60 * 1000;
    } else if (config.delayUntil) {
      // Calculate delay until specific datetime
      ...
      return {
        success: false,
        message: 'Wait duration not specified. Use delayMinutes, delayHours, delayDays, or delayUntil',
      };
    }
    ...
    // In a real workflow engine, this would schedule a job
    // For now, we'll just log and return success
    // The actual delay would be handled by the job queue
    return {
      success: true,
      message: `Wait action scheduled for ${Math.round(delayMs / 1000 / 60)} minutes`,
      data: {
        delayMs,
        delayMinutes: Math.round(delayMs / 1000 / 60),
        scheduled: true,
        note: 'In production, this would schedule a delayed job',
      },
    };
  } catch (error) {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to execute wait action',
    };
  }
}
 ...
 * Evaluates a condition and executes appropriate actions
 */
export async function executeBranch(
  ...
  try {
    if (!config.variable || !config.operator || config.value === undefined) {
      return {
        success: false,
        message: 'Branch action requires variable, operator, and value',
      };
    }
    ...
    switch (config.variable) {
      ...
      case 'client.tags':
        const clientTags = await prisma.client.findUnique({
          where: { id: context.clientId },
          select: { tags: true },
        });
      ...
      default:
        return {
          success: false,
          message: `Unknown variable: ${config.variable}`,
        };
    }
    ...
    switch (config.operator) {
      case 'equals':
        conditionResult = actualValue === config.value;
        break;
      ...
      case 'contains':
        conditionResult = Array.isArray(actualValue)
          ? actualValue.includes(config.value)
          : String(actualValue).includes(String(config.value));
        break;

      case 'greater_than':
        conditionResult = Number(actualValue) > Number(config.value);
        break;
      ...
      case 'in':
        conditionResult = Array.isArray(config.value) && config.value.includes(actualValue);
        break;

      default:
        return {
          success: false,
          message: `Unknown operator: ${config.operator}`,
        };
    }
    ...
    await prisma.activity.create({
      data: {
        ...
        metadata: JSON.stringify({
          ...
          actionCount: actionsToExecute?.length || 0,
        }),
      },
    });

    // Return information about the branch
    // In a full workflow engine, this would trigger the execution of the selected actions
    return {
      success: true,
      message: `Branch evaluated: ${conditionResult ? 'TRUE' : 'FALSE'}`,
      data: {
        conditionResult,
        actionsToExecute: actionsToExecute || [],
        actionCount: actionsToExecute?.length || 0,
        note: 'Actions would be executed by the workflow engine',
      },
    };
  } catch (error) {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to execute branch action',
    };
  }
}
...
export async function executeParallel(
  ...
  try {
    if (!config.actions || config.actions.length === 0) {
      return {
        success: false,
        message: 'Parallel action requires at least one action to execute',
      };
    }
    ...
    await prisma.activity.create({
      data: {
        ...
        metadata: JSON.stringify({
          ...
          actions: config.actions.map((a) => ({ type: a.type, config: a.config })),
        }),
      },
    });

    // In a full workflow engine, this would execute actions in parallel
    // For now, we just validate and return success
    const results = config.actions.map((action) => ({
      actionType: action.type,
      status: 'pending',
      note: 'Would be executed in parallel by workflow engine',
    }));

    return {
      success: true,
      message: `Parallel execution initiated for ${config.actions.length} action(s)`,
      data: {
        actionCount: config.actions.length,
        continueOnError: config.continueOnError || false,
        results,
        note: 'Actions would be executed in parallel by the workflow engine',
      },
    };
  } catch (error) {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to execute parallel action',
    };
  }
}
...
/**
 * Main dispatcher for flow control actions
 * Routes to the appropriate executor based on action type
 */
export async function executeFlowControlAction(
  ...
  switch (actionType) {
    case 'WAIT':
      return executeWait(config, context);
    case 'BRANCH':
      return executeBranch(config, context);
    case 'PARALLEL':
      return executeParallel(config, context);
    default:
      return {
        success: false,
        message: `Unknown flow control action type: ${actionType}`,
      };
  }
}
